...
| `희망진출국가(직접분석: 최대 2개, 없으면 공란 가능)` | 수출시장 분석 보고서 대상 국가. 쉼표, 슬래시, 줄바꿈으로 최대 2개까지 입력 | 모드별 |
...
`유망 시장 추천 보고서 생성`을 선택하면 기본적으로 추천 보고서 PDF만 저장합니다. 하위 옵션인 `추천 국가로 수출시장 분석보고서 생성 연동`을 켜면 추천 결과의 `국가별 시장 분석` 카드에서 카드별 가장 왼쪽 국가를 추출해 최대 2개 국가의 수출시장 분석보고서까지 이어서 생성합니다.
