...
- 백그라운드 실행: 저사양 VM 기준 기본 사용. Playwright Chromium headless shell 우선 사용, 실패 시 Microsoft Edge headless로 대체
...
- 수출액 규모
- 해당 품목 수출 경험
- HSCODE 6단위 또는 HSCODE 10단위
- 희망진출국가: `수출시장 분석 보고서 생성` 선택 시 필요
...
| `HSCODE 10단위(모르는 경우 생략 가능)` | 10자리 HS CODE. 6단위가 비어 있으면 앞 6자리를 자동 사용 | 선택 |
| `HSCODE 6단위` | 사이트 입력에 사용할 6자리 HS CODE. 10단위보다 우선 사용 | O |
| `수출품명(선택: 구체적으로 작성 권장)` | 수출품명 | X |
| `희망진출국가(직접분석: 기본 2개, 추천연동: 설정값만큼 입력 가능)` | 수출시장 분석 보고서 대상 국가. 쉼표, 슬래시, 줄바꿈으로 여러 국가 입력 가능 | 모드별 |
| `분석 제외 국가(추천보고서: 없으면 공란 가능)` | 유망 시장 추천 보고서에서 제외할 국가. 여러 국가는 쉼표, 슬래시, 줄바꿈으로 입력 | X |

기존 영문 컬럼(`hs_code`, `product_name`, `export_scale`, `export_experience`, `target_country`, `excluded_countries`)도 지원합니다.
...
2. GUI의 `보고서 생성 방식`에 따라 `희망 국가 직접 분석` 또는 `유망 시장 추천 받기` 선택
3. 엑셀 한 행의 값 입력. 추천 보고서에서는 분석 제외 국가가 있으면 국가별로 입력 후 `+`를 클릭
4. 보고서 생성 버튼 클릭
...
`수출시장 분석보고서 생성`은 희망진출국가 앞 2개 국가의 보고서를 생성합니다.

`유망 시장 추천 보고서 생성`을 선택하면 기본적으로 추천 보고서 PDF만 저장합니다. 하위 옵션인 `추천 국가로 수출시장 분석보고서 생성 연동`을 켜면 추천 보고서를 먼저 생성한 뒤, 엑셀의 희망진출국가를 우선 사용하고 부족한 개수만 추천 결과의 `국가별 시장 분석` 카드에서 채워 수출시장 분석보고서를 이어서 생성합니다. 이때 GUI의 `분석보고서 수` 또는 CLI의 `--direct-report-count`로 생성할 수출시장 분석보고서 수를 조정할 수 있으며 기본값은 2개입니다.
...
추천 보고서 생성 후 수출시장 분석보고서까지 이어서 생성하는 경우에는 `logs\report_tasks.xlsx`에 추천 보고서와 국가별 수출시장 분석 작업을 따로 기록합니다. 자동 재시도 또는 `실패 행 다시 실행` 시 이미 완료된 작업은 건너뛰고 남은 작업부터 이어서 실행합니다.
...
프로그램 실행 시 기본값은 저사양 VM에 맞춘 백그라운드 실행입니다. GUI에서는 `백그라운드 실행` 스위치를 끄면 브라우저 화면을 표시할 수 있고, CLI에서는 `--no-headless`로 화면 표시 모드를 사용할 수 있습니다. 화면 표시 모드에서는 Microsoft Edge를 먼저 실행합니다. 백그라운드 실행 모드에서는 Windows 원격 VM에서 빈 브라우저 창이 뜨는 현상을 줄이기 위해 Playwright Chromium headless shell을 먼저 실행하고, 설치되어 있지 않으면 Microsoft Edge headless로 대체합니다. Edge 실행에 실패하면 macOS 등 로컬 테스트 편의를 위해 Playwright 기본 Chromium으로 한 번 더 시도합니다.
...
GUI에서 `실패 행 다시 실행` 버튼을 누르면 `logs\failed_rows.xlsx` 기준으로 실패한 행만 다시 실행합니다.

일반 실행 중 실패한 항목은 재시도 대기열에 올라가 1회 다시 처리되며, 자동 재시도 후에도 실패한 항목만 실패 행 다시 실행 대상에 남습니다.

유망 시장 추천 보고서와 연동된 수출시장 분석보고서는 `logs\report_tasks.xlsx`를 기준으로 완료 작업을 확인합니다. 추천 보고서나 일부 국가 보고서가 이미 성공했다면 실패한 나머지 작업만 이어서 실행합니다.
...
- HSCODE 6단위가 있으면 해당 값을 우선 사용합니다. 6단위가 비어 있고 10단위가 있으면 앞 6자리를 자동 사용합니다.
